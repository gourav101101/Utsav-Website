...
const ProductsManager: React.FC = () => {
  ...
  const [newProduct, setNewProduct] = useState({ title: '', price: '', category: '', images: [] as string[] });
  ...
  async function handleCreateProductSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const imagesArr = (newProduct.images || []).map(s => s.trim()).filter(Boolean);
      await createProduct({ title: newProduct.title, price: newProduct.price, category: newProduct.category, images: imagesArr, description: '' });
  ...
  return (
    <div id="products" className="bg-white p-4 rounded shadow">
          ...
          <div className="col-span-1 md:col-span-4 space-y-2">
            {newProduct.images.map((img, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <button className="px-3 py-1 rounded border" onClick={() => setNewProduct({ ...newProduct, images: newProduct.images.filter((_,i)=>i!==idx) })}>Remove</button>
              </div>
            ))}
            <div>
              <button className="px-3 py-2 rounded bg-primary text-white" onClick={() => setNewProduct({ ...newProduct, images: [...newProduct.images, ''] })}>Add Image</button>
            </div>
          </div>
      ...
      <div id="products-list" className="bg-white p-4 rounded shadow">
                  ...
                  {products.map((p, idx) => (
                    <tr key={p._id || p.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-2 align-top flex items-center gap-3">
                        <img src={p.image || '/placeholder.png'} alt="thumb" className="w-14 h-10 object-cover rounded" />
                        <div>
              ...
              {products.map((p, idx) => (
                <div key={p._id || p.id} className="border rounded p-3 bg-white shadow-sm">
                  <div className="flex items-start gap-3">
                    <img src={p.image || '/placeholder.png'} alt="thumb" className="w-20 h-14 object-cover rounded" />
